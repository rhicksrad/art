import { axisBottom, axisLeft, bin, extent, max, scaleLinear, select, type Bin } from 'd3';
...
export const createHistogram = ({ values, bins }: HistogramProps): HistogramHandle => {
  ...
  const render = (series: number[]): void => {
    ...
    const histogram = bin<number, number>().domain(domain).thresholds(thresholdCount);
