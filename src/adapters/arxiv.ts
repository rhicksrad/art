...
export const toItemCards = (atomXml: string): ItemCard[] => {
  ...
  return entries.map((entry, index) => {
    const title = normalizeWhitespace(entry.querySelector("title")?.textContent) ?? `arXiv entry #${index + 1}`;
    ...
    const id = normalizeWhitespace(entry.querySelector("id")?.textContent) ?? href ?? `arxiv-${index}`;
